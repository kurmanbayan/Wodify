...
class MusclesViewCell: UITableViewCell {
    ...
    private func updateUI() {
        ...
        var values = user!.muscles
        values.append(user!.muscles.last!)
        values.insert(user!.muscles.first!, at: 0)
        setChart(Values: values)
    }
    func setChart (Values: [Double]) {
        let color = UIColor(red: 0.9, green: 0.13, blue: 0.03, alpha: 1)
